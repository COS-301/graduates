...
describe('RequestForAccessComponent', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      ...
      imports: [MatDividerModule, HttpClientModule, NgxsModule.forRoot([AccessState], { developmentMode: true })]
    })
    .compileComponents();
  ...
  it('should initialize resource state', () => {
    store.select( state => state.accessState.accessGranted[0]).subscribe({
      next: (status) => {
