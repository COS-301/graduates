...
export class ApiAdminConsoleServiceFeature {
    ...
    async addPermission(user: User, permission: Prisma.UserPermissionsCreateInput): Promise<void> {
        return this.commandBus.execute(new AddPermissionCommand(user, permission));
    }

    async addRole(user: User, permission: Prisma.UserRoleCreateInput): Promise<void> {
        return this.commandBus.execute(new AddRoleCommand(user, permission));
    }

    async removePermission(userID: string, permission: Prisma.UserPermissionsDeleteArgs): Promise<void> {
        return this.commandBus.execute(new RemovePermissionCommand(userID, permission));
    }

    async removeRole(userID: string, permission: Prisma.UserRoleDeleteArgs): Promise<void> {
